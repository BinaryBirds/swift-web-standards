...
struct MediaTests {
    ...
    func queryValues() {
        ...
        #expect(
            Media.Query.orientation(.landscape).rawValue
        ...
        #expect(
            Media.Query.displayMode(.standalone).rawValue
    ...
    func additionalValues() {
        ...
        #expect(
            Media.Query.resolution("300dpi").rawValue == "(resolution: 300dpi)"
        )
        #expect(Media.Query.grid(.yes).rawValue == "(grid: 1)")
        #expect(
            Media.Query.scan(.progressive).rawValue == "(scan: progressive)"
        )
        #expect(
            Media.Query.deviceWidth(1024.px).rawValue
                == "(device-width: 1024px)"
        )
    }
    ...
    func moreQueries() {
        ...
        #expect(
            Media.Query.maxResolution("2dppx").rawValue
    ...
    func deviceAndAspectValues() {
        #expect(
            Media.Query.deviceMinWidth(480.px).rawValue
                == "(min-device-width: 480px)"
        )
        #expect(
            Media.Query.deviceMaxHeight(900.px).rawValue
                == "(max-device-height: 900px)"
        )
        #expect(
            Media.Query.minAspectRatio("4/3").rawValue
                == "(min-aspect-ratio: 4/3)"
        )
        #expect(
            Media.Query.maxAspectRatio("21/9").rawValue
